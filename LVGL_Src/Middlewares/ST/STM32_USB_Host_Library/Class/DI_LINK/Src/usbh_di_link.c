//Implementing DisplayLink Class for USB Host library.
//EP0 (control EP) is used fo configuring device
//EP1 (Out) is used for configuring deviceand sending image data
//EDID values are read, but do not processed in this code
//Device is ready to send image data when (DI_LINK_Handle->state == DI_LINK_IDLE_STATE)

/* Includes ------------------------------------------------------------------*/
#include "usbh_di_link.h"
#include "di_link_send_data.h"
#include <yfuns.h> // remove
